...
//! Keeping one implementation is the point, which is also why this sits at the crate root rather
//! than inside `native`: the passes layer must not depend on `native`, and a second copy over there
//! would be exactly the duplication this module exists to prevent. The obvious alternative -- a dominator *set* per block --
