//! Simdgroup matrix AIR call lowering: the 8x8 and 16x16 multiply-accumulate families, the
//! block load/store address model, and the lane/fragment mapping the 16x16 fragments use.
//!
//! The subgroup shuffle primitives these build on live beside this in `shuffle`.
...
///
/// **Device-measured** (Apple M3 Max / macOS 26.5.2), not asserted. A hand-written AIR module
/// calling each of the four ABI symbols was assembled with `xcrun metal -x ir` and run against a
/// simulation of the exact shuffle schedule below. All 32 lanes x 2 cells agree on every run: three
/// designed input sets (distinct A with all-ones B, all-ones A with distinct B, and all three
/// operands distinct) plus ten random integer trials for `f32matmad{8x8,4x4}`, and six
/// exactly-representable integer trials for `f16matmad{8x8,4x4}`. That pins the storage mapping,
/// the operand order, and the accumulate direction together, because the same probe distinguishes
/// A-B from B-A and a row mapping from a column mapping.
...
///
/// **This mapping is NOT device-verified, and cannot be on the hardware here.** The sibling AGX2
/// 8x8 ABI it is modelled on IS -- see [`lower_agx2_matmad`], measured against a hand-assembled AIR
/// probe -- but the same probe for the 16x16 family is refused at pipeline creation by an M3 Max:
/// `simdgroup_matrix<T,16,16x16> operations are supported by GPUFamily10 and later`. So the 43
/// corpus sources reaching `air.simdgroup_matrix_16x16x16_multiply_accumulate` and the 17 reaching
/// its widening `s`/`u` forms translate without a device oracle behind their lane model. Anything
/// asserted about this mapping is derivation, not measurement; a GPUFamily10 device would settle it
/// in one run of that probe.
...
/// input fragment and descriptor 9 denotes an i32 accumulator/result fragment. Both readings are
/// UNVERIFIED: this intrinsic cannot be device-measured on an M3 Max, where a hand-assembled AIR
/// module naming it takes the Metal compiler service down with `XPC_ERROR_CONNECTION_INTERRUPTED`. Each input fragment
...
fn matrix16_to_accumulator(
    ...
    match kind {
        ...
        // The accumulator is f32 only when the result or some operand is wider than f16; an
        // ALL-half MAC accumulates at half, and then this widening is the identity. `OpFConvert`
        // requires the two widths to differ, so take the same guard the result lane takes rather
        // than emitting an instruction the owned module rejects.
        Matrix16Element::F16 => {
...
/// How `air.simdgroup_matrix_8x8_{load,store}` reaches element `[row][col]` from its pointer
/// operand. The operand is one of exactly two shapes, and they need different access chains:
///
/// * **`Element`** -- the pointer already addresses one matrix element, which is what a
///   `device const float *` parameter is. Stride it with `OpPtrAccessChain`.
/// * **`Block`** -- the pointer addresses the whole `[N x element]` array, which is what a
///   `threadgroup float ta[128]` declared inside the kernel is: AIR passes the array symbol itself,
///   never a GEP to its first element. Index INTO it with an ordinary `OpAccessChain`.
///   `OpPtrAccessChain` is wrong here twice over -- it would stride by whole 128-element arrays, and
///   on a `Workgroup` pointer it needs the `VariablePointers` capability, which this translator does
///   not declare (only `VariablePointersStorageBuffer`).
enum MatrixBlockBase {
    Element,
    /// The pointer-to-element type the `OpAccessChain` result carries.
    Block(Word),
}

/// Resolve the pointer operand of a `simdgroup_matrix` block access against the matrix element type,
/// or name the mismatch. `what` is `"load"`/`"store"` and `whose` the operand the element type came
/// from, so the two callers keep their own wording.
fn simdgroup_matrix_block_base(
    ...
    Err(format!(
        "air.simdgroup_matrix {what} pointee does not match {whose} element type"
    ))
}

/// The storage class of an `OpTypePointer`, or `None` for anything else.
...
/// One term of a matrix element index, kept symbolic so a statically known descriptor folds to a
/// single constant instead of an `OpIMul`/`OpIAdd` chain per cell. Every descriptor in the corpus but
/// two is a literal, and an arithmetic guard that cannot fire must not be emitted.
#[derive(Clone, Copy)]
enum Term {
    Known(i64),
    Value(Word),
...
/// The address arithmetic of an `air.simdgroup_matrix_8x8_{load,store}`, decoded from its two
/// trailing descriptor vectors. **Device-measured** on an Apple M3 Max (macOS 26.5.2):
///
/// ```text
/// element(row, col) = base[(row + origin.y) * strides.y + (col + origin.x) * strides.x]
/// ```
///
/// `strides` is the SECOND `<2 x i64>` descriptor and `origin` the third operand. One formula covers
/// every spelling the Metal frontend emits, because `transpose_matrix` is encoded by SWAPPING the two
/// components of the stride vector AND of the origin together:
///
/// | MSL | strides | origin |
/// |---|---|---|
/// | `simdgroup_load(m, p, 16)` | `<1, 16>` | `<0, 0>` |
/// | `simdgroup_load(m, p, 16, ulong2(3, 2))` | `<1, 16>` | `<3, 2>` |
/// | `simdgroup_load(m, p, 16, ulong2(0, 0), true)` | `<16, 1>` | `<0, 0>` |
/// | `simdgroup_load(m, p, 16, ulong2(3, 2), true)` | `<16, 1>` | `<2, 3>` |
///
/// so there is no transpose flag to read and no branch to write. The FIRST descriptor is the region
/// extent along each axis (`<16, 8>` / `<8, 16>` above) and no address depends on it.
///
/// The lowering this replaced read `strides` nowhere: it took the leading dimension from component 0
/// of the first descriptor, which is the real row stride only in the untransposed spelling, and
/// ignored the origin entirely. A transposed `simdgroup_load` at leading dimension 32 read
/// `base[row * 8 + col]`.
...
impl MatrixBlockAddress {
    /// `(constant + origin) * stride`, folded whenever both ends are known.
    fn axis(
        ctx: &mut Ctx,
        insts: &mut Vec<Instruction>,
        idx_ty: Word,
        ...
        let offset = match origin {
            Term::Known(value) => Term::Known(i64::from(constant) + value),
            Term::Value(id) => {
                let base = ctx.const_int_of(idx_ty, i64::from(constant));
                Term::Value(binary(ctx, insts, Op::IAdd, idx_ty, base, id))
            }
        };
        match (offset, stride) {
            (Term::Known(a), Term::Known(b)) => Term::Known(a * b),
            (Term::Known(0), _) => Term::Known(0),
            (offset, Term::Known(1)) => offset,
            (offset, stride) => {
                let (a, b) = (offset.id(ctx, idx_ty), stride.id(ctx, idx_ty));
                Term::Value(binary(ctx, insts, Op::IMul, idx_ty, a, b))
            }
        }
    }

    /// The element index of matrix cell `(row, col)`.
    fn index(&self, ctx: &mut Ctx, insts: &mut Vec<Instruction>, row: u32, col: u32) -> Word {
        let down = Self::axis(
            ctx,
            insts,
            self.idx_ty,
...
// Element pointer for matrix cell (row, col). See [`MatrixBlockAddress`] for the index and
// [`MatrixBlockBase`] for which access chain each pointer shape takes.
...
/// Component `lane` of a `<2 x i64>` descriptor, as a literal when the vector is a module constant
/// (`OpConstantNull` or an `OpConstantComposite` of `OpConstant`s) and as an extracted value
/// otherwise.
...
/// Decode the `(strides, origin)` operand pair of a `simdgroup_matrix` block access. `strides` is the
/// descriptor at `descriptors[1]`; `origin` is optional because the AIR ABI grew it and a two-operand
/// call has none.
...
// `air.simdgroup_matrix_8x8_load(ptr, extents, strides, origin)` -> a 64-lane row-major composite
// gathered from memory; see [`MatrixBlockAddress`] for the element index.
pub(in crate::passes) fn lower_simdgroup_matrix_8x8_load(
    ...
    let address = simdgroup_matrix_address(ctx, &mut insts, args[2], args.get(3).copied())?;
    let mut result_lanes = Vec::with_capacity(64);
    for row in 0..8u32 {
        for col in 0..8u32 {
...
// `air.simdgroup_matrix_8x8_store(M, ptr, extents, strides, origin)` -> scatter the 64-lane
// row-major matrix into memory; see [`MatrixBlockAddress`] for the element index.
pub(in crate::passes) fn lower_simdgroup_matrix_8x8_store(
    ...
    let mut insts = Vec::new();
    let address = simdgroup_matrix_address(ctx, &mut insts, args[3], args.get(4).copied())?;
    for row in 0..8u32 {
        for col in 0..8u32 {
